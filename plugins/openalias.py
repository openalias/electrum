...
class Plugin(BasePlugin):
    ...
    def resolve(self, url):
        ...
        self.print_error('Attempting to resolve OpenAlias data for ' + url)
    ...
    def validate_dnssec(self, url):
        ...
        for i in xrange(len(parts), 0, -1):
            ...
            if response.rcode() != dns.rcode.NOERROR:
                self.print_error("Encountered a DNS query error when checking nameservers for record ")
                return 0
            ...
            response = dns.query.udp(query, ns, 3)
            if response.rcode() != 0:
                self.print_error("Encountered a DNS query error when checking DNSKEY records")
                return 0
            ...
            if len(answer) != 2:
                ...
                if len(rrsig) != 1:
                    self.print_error("Failed to retrieve DNSKEY and RRSIG records")
                    return 0
            ...
            try:
                ...
                self.print_error("The record failed DNSSEC validation")
                return 0
